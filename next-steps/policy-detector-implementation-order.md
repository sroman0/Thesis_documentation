...
| 20 | `rules/detectors/*.yaml` | Fornire detector demo | Completato: point anomalies e primo collective `privilege-exec-chain` con finestra breve | Formato YAML definito | Caricati da `pkg/detectors/yaml` |
| 21 | `rules/policies/*.yaml` | Fornire policy demo | Completato: policy preset per full demo, collective privilege-exec, point process security, sensitive file access e kernel module activity | `pkg/policy` | Caricate da CLI |
