...
| 3 | `pkg/cmd/project.go` | Collegare config, runtime e detector layer | Completato: prepara policy, analizza il catalogo detector, applica la selezione policy-driven e costruisce solo i detector scelti prima di avviare eBPF | `pkg/config`, `pkg/policy`, `pkg/detectors` | Coordina `pkg/ebpf/project.go` e output |
| 4 | `pkg/policy/policy.go` | Definire il modello interno di policy | Completato: modello eventi e scope più `DetectorSelector` per ID, severity, MITRE, tag e stato | `pkg/detectors` | Usato da loader, manager e selettore detector |
| 5 | `pkg/policy/loader.go` / `detector_selector.go` | Caricare e compilare policy dichiarative | Completato: caricamento YAML, normalizzazione metadata e selezione detector validata prima della costruzione runtime | `gopkg.in/yaml.v3`, `pkg/detectors` | Produce policy e insieme detector selezionato |
...
| 9 | `pkg/detectors/yaml/schema.go` | Definire il formato YAML dei detector | Completato: schema esterno con `id`, `name`, `requires`, `consumes`, `conditions`, `steps`, `output`, `threat`, `stateful`, `window`, `group_by`, `tags`; `group_by` e' una lista di strategie componibili | `gopkg.in/yaml.v3` nei test | Letto dal parser detector |
| 10 | `pkg/detectors/yaml/parser.go` | Convertire YAML in detector definition | Completato: parsing e normalizzazione, validazione eventi e correlazione, rifiuto strategie sconosciute/duplicate/non risolvibili; alias `pid`/`host_pid` convertiti in `process` | `pkg/detectors`, `pkg/events`, `gopkg.in/yaml.v3` | Produce definizioni detector validate |
| 11 | `pkg/detectors/yaml/detector.go` / `correlation.go` | Implementare detector e correlazione YAML runtime | Completato: condizioni compilate, sequenze corte, piani `process`, `process_tree`, `resource`, `cgroup`, chiavi composite, pruning e limite stato; `user_session` differita | `pkg/detectors`, `pkg/bufferdecoder`, `pkg/events` | Pronto per registry/dispatcher |
...
| 14 | `pkg/detectors/engine.go` | Orchestrare registry e dispatcher | Completato: `Engine`, `NewEngine`, `Register`, `Init`, `ProcessEvent`, `Flush`, `Metrics`; metriche minime, errori detector isolati e dedup temporale degli alert ripetuti | `pkg/detectors` | Punto unico per futuro runtime e output alert |
...
| 20 | `rules/detectors/*.yaml` | Fornire detector demo | Completato: point anomalies, collective process-tree e demo `temp-script-write-exec` correlata per `dev:inode` | Formato YAML definito | Caricati da `pkg/detectors/yaml` |
| 21 | `rules/policies/*.yaml` | Fornire policy demo | Completato: preset per eventi e `mitre-privilege-collective.yaml` per selezione detector basata su metadata | `pkg/policy` | Caricate da CLI |
