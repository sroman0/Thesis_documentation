...
|---|---|---|---|---|---|
| Vesuvius targets Rocky Linux 8.10 on `4.18.0-553.137.1.el8_10.x86_64`, x86_64. | **Verified as the declared and tested target** | `demo_project/README.md`, **Requirements** table; `documentation/implementation/performance.md`, **Result of 10 June 2026**; `documentation/daily/2026-05-19.md`, recorded `uname -a`. Build defaults are in `pkg/config/config.go`, `Default`. | “The prototype was developed for and evaluated primarily on Rocky Linux 8.10 with kernel `4.18.0-553.137.1.el8_10.x86_64` on x86_64.” | Chapter 3 should explain RHEL backports, BTF availability, CO-RE, and target-driven hook adaptations. Chapter 5 should state the exact test environment. | Do not generalize this into compatibility with every Rocky Linux, RHEL, 4.18, kernel, or architecture. The target is documented rather than enforced by a runtime compatibility gate. |
...
| The decoder normalizes the custom C event format into typed Go events. | **Verified** | `pkg/bufferdecoder/protocol.go`: `EventContext`, `Event`; `pkg/bufferdecoder/eventsreader.go`: `DecodeEvent`, argument-record decoders; `pkg/bufferdecoder/event_args.go`: typed argument model; `pkg/events/spec.go`: event argument schemas; commit `163ad72`: long-record offset correction and regression coverage. | “Userspace decodes a fixed event context and schema-driven argument records into a common Go event model used by output, policy matching, and detectors.” | Chapter 4 should describe the 136-byte context contract, argument record format, supported payload types, explicit bounds checks and unknown-ID handling. Chapter 5 should include the long-path regression test. | The decoder is tested and supports scalar, string, string-array, argument-array, credential, and socket-address payloads. The eBPF serializer now bounds offsets explicitly instead of masking them by `ARGS_BUF_SIZE - 1`, which was invalid because the 32000-byte buffer is not a power of two. “Robust” should still be reserved for broader malformed-input, fuzz and compatibility evidence. `MatchedPolicies` in the wire context remains outside the active policy path. |
...
| The detector engine supports local collective anomalies. | **Verified for ordered short-window sequences** | `pkg/detectors/definition.go`: stateful mode and 2-second default/5-second maximum windows; `pkg/detectors/yaml/detector.go`: sequence-state lifecycle; `pkg/detectors/yaml/correlation.go`: compiled correlation plans; collective rules include process-based chains and `temp_script_write_exec.yaml`. | “The local detector engine can correlate ordered event sequences within short time windows to represent selected collective anomaly patterns.” | Chapter 3: rationale for local collective detection. Chapter 4: state lifecycle, grouping strategies and state bounds. Chapter 5: sequence tests, expiry behavior, memory/CPU effects and false positives. | This is not general collective-anomaly discovery, statistical anomaly detection or distributed correlation. Incomplete sequence state is bounded by time and by a maximum of 4096 entries per detector. Contextual anomalies and cluster-wide state are explicitly out of scope. |
| Collective detectors support configurable local correlation. | **Verified** | `pkg/detectors/yaml/correlation.go`: compiled strategies for `process`, `process_tree`, `resource`, `cgroup` and ordered composite keys; `pkg/detectors/yaml/correlation_test.go`: PID reuse, parent-child, cross-process resource, cgroup and composite-key cases; `temp_script_write_exec.yaml`: resource-based cross-process example. | “Sequence detectors can correlate events through stable process identity, an immediate parent relationship, file-resource identity, local cgroup identity, or composite combinations of these dimensions.” | Chapter 4 should explain key construction, PID-reuse protection, `dev:inode` resource identity, cgroup scope, composite-key semantics and validation. | `process_tree` is limited to the same process or one immediate parent-child relation. `resource` requires `dev` and `inode`; cgroup identity is node-local and is not a Kubernetes workload identity. `user_session` is explicitly rejected until a stable session identifier is available. |
...
6. **Local correlation is broader than process-tree correlation but remains
   bounded.** The engine now supports process, immediate parent-child,
   file-resource, local cgroup and composite identities. It does not maintain a
   persistent process graph, stable user-session identity or cluster-wide state.
...
YAML point and short-window collective detectors, configurable local
