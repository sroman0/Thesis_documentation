...
$\rightarrow$ selezione probe $\rightarrow$ load/attach $\rightarrow$ ring buffer
...
\node[box, below=of selection] (runtime) {
  ...
  carica collection, apre la ring buffer e attacca solo i probe selezionati
};
\node[box, below=of runtime] (kernel) {
  \textbf{Kernel data path}\\
  gli hook eBPF selezionati emettono eventi nella mappa \texttt{events}
};
\node[box, below=of kernel] (decoder) {
  \textbf{Read + decode}\\
  \texttt{ringbuf.Read()} $\rightarrow$ \texttt{bufferdecoder.DecodeEvent()}:
...
Il runtime Go carica l'oggetto eBPF, apre la ring buffer e mantiene i link agli
hook kernel. Dopo il merge con il branch networking, l'implementazione usa
\texttt{github.com/aquasecurity/libbpfgo}, quindi il Makefile prepara anche
\texttt{libbpf} e i flag CGO necessari. I link vengono chiusi in modo ordinato
durante il cleanup.
