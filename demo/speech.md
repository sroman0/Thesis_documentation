...
La copertura attuale è molto più ampia rispetto alla prima demo. Sul lato
processi osserviamo `execve`, `execveat`, `fork`, `vfork`, `clone`, exec fallite
e cambi di UID/GID tramite la famiglia `set*id`. Sul lato memoria abbiamo
`mmap`, `mprotect`, `pkey_mprotect`, `memfd_create`, `process_vm_readv` e
`process_vm_writev`. Per molti di questi eventi salviamo gli argomenti
all'ingresso della syscall e produciamo l'evento all'uscita, includendo quindi
successo, errno, file descriptor o numero di byte trasferiti.
...
Nell'ultima evoluzione abbiamo aggiunto anche eventi più vicini all'hardening
kernel: `security_bpf`, `security_bpf_map`, `security_bpf_prog`,
`security_kernel_read_file`, `security_kernel_post_read_file`,
`module_load`, `module_free`, `do_init_module`, `proc_create`,
`register_kprobe` e `kallsyms_lookup_name`. Questi hook non servono solo a
vedere processi utente, ma anche comportamenti più interni: caricamento moduli,
letture fatte dal kernel, registrazione dinamica di kprobe, entry procfs create
da moduli e lookup di simboli kernel.
